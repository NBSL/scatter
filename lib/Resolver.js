...
Resolver.prototype._expandPaths = function(globs, basePath) {
  ...
  _.each(globs, function(globbedPath) {
    if (_.isPlainObject(globbedPath)) {
      globbedPath.path = path.join(basePath, globbedPath.path);
      paths.push(globbedPath);
    } else {
      if(basePath) {
...
Resolver.prototype.registerParticles = function(particlesPaths, basePath) {
  ...
  _.each(particlesPaths, function(particlePath) {
    ...
    if (typeof particlePath === 'object'){
      if (!particlePath.path) {
        throw new Error("Scatter requires a path to be set when adding `subparticles` in the package.json file.");
      }
      descriptor = particlePath;
      particlePath = descriptor.path;
      console.log(descriptor);
    } else if(fs.existsSync(path.join(particlePath, "package.json"))){
      descriptor = require(path.join(particlePath, "package.json")).particles;
    } else {
      ...
      descriptor = require(descriptorFileName);
    }
